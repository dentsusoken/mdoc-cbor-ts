...
const mdocObj = deviceResponseSchema.parse(
  decode(Buffer.from(mdoc, 'base64url'))
);
...
describe('MSOVerifyHandlerImpl', () => {
  ...
  describe('verify', () => {
    ...
    it('should verify IssuerSigned mdoc', async () => {
      const mdocVerifyHandler = new MdocVerifyHandlerImpl(schemas);
      const issuerSignedString = encode(
        mdocObj.documents?.[0]?.issuerSigned
      ).toString('base64url');
      const result = await mdocVerifyHandler.verify(issuerSignedString);
      expect(result.valid).toBe(true);
    });
  });
});
